...
1. Check for previous completion: if `.brr-complete` exists, this is a re-run. Note this, then delete stale signal files: `rm -f .brr-complete .brr-needs-approval`. If a re-run was detected and `COMPILE.md` exists, rewrite `COMPILE.md` keeping only the `## Guardrails` section (drop `## Spec Queue`). This causes Phase 1 to route to Phase 2, which rebuilds the queue from scratch while preserving learned patterns.
...
**B) `## Spec Queue` has at least one line starting with `- `** → Phase 3 (Compile Spec)
**C) `## Spec Queue` exists but has no lines starting with `- `** → Phase 4 (Complete)
...
3. Read the `## Dependencies` section of every spec. Build a dependency graph: if spec A lists spec B in its dependencies, A must come after B in the queue.
4. Topologically sort specs so dependencies come before dependents. If cycles exist, break them by placing the spec with fewer incoming edges first and log the break as an HTML comment (e.g., `<!-- cycle broken: A ↔ B, A placed first -->`). Within the same depth, group specs that share dependencies adjacently. Leaf specs (no spec-to-spec dependencies) go first; specs with the most dependencies (that depend on many other specs) go last.
5. If `COMPILE.md` already exists, preserve the `## Guardrails` section. Write or replace only the `## Spec Queue` section. If the file does not exist, create it with both sections:
...
- For each remaining spec compliance FAIL: determine whether it explicitly references functionality from another spec still in `## Spec Queue` (uncompiled). If yes → DEFERRED. If no (genuine implementation failure) → include in `.brr-needs-approval`.
...
1. `git add COMPILE.md && git commit -m "docs(compile): <action> <spec-name>"`
   ...
   If there are DEFERRED items, append them to the commit body:
   ```
   DEFERRED: criterion text (depends on other-spec.md)
   ```
2. Exit.

## Phase 4: Complete

1. Search for DEFERRED items: run `git log --all --grep="DEFERRED:" --format="%s%n%b"` to find compile commits with deferred criteria. If any are found, create `.brr-needs-approval` listing each deferred item with the message: "These acceptance criteria were deferred during compilation because they depended on specs that had not been compiled yet. All specs are now compiled. Please verify these items manually or re-queue the affected specs." Exit.
