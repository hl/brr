...
Installed brr binaries include this guide. Run:

```bash
brr instructions
```

Use the output when you are an agent setting up brr in a project. Prefer creating project-local files so the workflow is versioned with the codebase and repeatable for other agents.
