...
3. `brr init` appends brr's runtime artifacts (`.brr-complete`, `.brr-needs-approval`, `.brr.lock`, `.brr-workflow-state.json`) to `.gitignore`. If `.gitignore` does not exist, it is created.
