...
- [ ] `brr <prompt> --notify` sends a desktop notification on each of the
      four terminal events listed in requirement 1.
- [ ] `brr <prompt>` (without `--notify`) sends no notifications.
- [ ] Ctrl+C stop does not trigger a notification.
- [ ] A notification failure (e.g. `notify-send` missing on Linux) is logged
